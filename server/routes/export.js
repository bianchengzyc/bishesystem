...
// @route   GET /api/export/yolo
// @desc    Export annotations in YOLO format
// @access  Public (temporarily for testing)
router.get('/yolo', async (req, res) => {
  try {
    console.log('=== Export YOLO request received ===');
    console.log('Query params:', req.query);

    // 获取查询参数
    const { image_ids, include_images = 'true' } = req.query;
    ...
    // 生成临时ZIP文件路径
    const zipFilename = `yolo_export_${Date.now()}.zip`;
    ...
    try {
      ...
      return res.json({
        success: true,
        message: 'YOLO export test successful',
